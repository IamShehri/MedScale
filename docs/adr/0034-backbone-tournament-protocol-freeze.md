...
6. `report-validation-contract.json` is normative for cross-record and arithmetic invariants that JSON Schema cannot express: exact activated bindings, canonical corpus item-ID membership, unique candidate identities, terminal error accounting, gate recomputation, winner↔candidate/gate linkage, and exact-tie membership.
