...
- require the PR evidence object keyset/version and replay-snapshot keyset/version exactly as defined in `acceptance.md`; unknown/duplicate/missing keys or wrong constant values fail closed. For a selected PR whose evidence is absent or invalid, derive the exact ordered `evidence_parse_status` from `acceptance.md`, store `evidence = null`, and continue deterministic snapshot construction while classifying the episode `BLOCKED`;
...
5. `main_tip`, history digest/count, result refs, sibling claim refs, selected PR evidence status, and valid evidence bytes must otherwise remain identical. Any other change => `BLOCKED`, no result-ref creation, no frozen-content read. The claim remains permanent replay evidence and the episode is burned/non-reusable.
...
- read the authoritative hosting commit-verification object deterministically even when `verification.signature` or `verification.payload` is `null`; PASS still requires `verification.verified = true` and both source texts available/non-null for exact hashing;
...
If any result-adoption verification predicate fails, publish `outcome = CANONICAL_ADOPTION_VERIFICATION_FAILED` with exactly the closed mapped `failed_checks` codes required by `acceptance.md`; the successor candidate stays inactive/unusable. Structural record errors and adoption-record publication/path/tree-scope errors remain validation failures outside the record's self-contained failure-code set. Do **not** rewrite the immutable terminal receipt or claim that it changed from `CONSUMED` to `BLOCKED`.
