...
The result package must also contain `preflight-result-manifest.json`.

To avoid a verdict/manifest digest cycle, construct the binding in this exact order:
...
3. Construct `manifest_binding_core` as canonical JSON under `MESC-BT-PREFLIGHT-CANONICAL-JSON-V1` containing exactly:
   ...
   - `successor_candidate_id = FD-MESC-BT-EXEC-1-CANDIDATE-V2`;
   - `successor_candidate_path = execution-authorization-candidate.md` under the preflight result directory.
4. `MANIFEST_BINDING_CORE_SHA256 = SHA256(canonical_manifest_binding_core_bytes)`.
5. Generate `preflight-verdict.md` in UTF-8 without BOM, LF endings, exactly one final LF, and require it to contain the exact `MANIFEST_BINDING_CORE_SHA256`, manifest ID, authorization merge SHA/tree, and terminal preflight state.
6. Compute the exact full-file SHA-256 of `preflight-verdict.md`.
7. Generate `preflight-result-manifest.json` as canonical JSON under Section B. It must contain the complete `manifest_binding_core` object, its SHA-256, and an `artifacts` map containing the exact path, SHA-256, and byte length of **all four core outputs**, including the final verdict file.
8. The manifest MUST NOT contain its own file SHA-256. Its exact full-file SHA-256 is computed externally and published in `consumption-receipt.json` and in the preflight result PR description.

A missing/mismatched core hash, output hash, byte length, path, or verdict reference => `BLOCKED`.
...
If Sections A–F pass, the result may create exactly one successor inactive candidate:
