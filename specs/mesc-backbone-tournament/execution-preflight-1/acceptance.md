...
`PRECLAIM_REPLAY_SNAPSHOT` is canonical JSON under Section B and has exactly these top-level keys: `snapshot_version = MESC-BT-PREFLIGHT-REPLAY-SNAPSHOT-V1`, `authorization_merge_sha`, `activation_receipt_id`, `main_tip`, `history_reachable_commit_count`, `history_graph_sha256`, `result_refs`, `claim_refs`, and `selected_prs`. No other top-level key is permitted.
