...
The worker must construct the canonical `PRECLAIM_REPLAY_SNAPSHOT` exactly as defined in `acceptance.md`, including its exact top-level keyset, constant-valued `snapshot_version`, deterministic array record schemas/order, and history-graph digest. Repeat all four exhaustive searches immediately before claim creation and require byte-identical canonical snapshot bytes. After atomic claim creation, repeat them again before creating `RESULT_REF`; the only permitted relevant delta is appearance of the exact `CLAIM_REF` at `AUTHORIZATION_MERGE_SHA`; all other fields, including `main_tip`, must remain identical. Any other drift => `BLOCKED`, no result-ref creation, no frozen-content read. The successful claim itself remains permanent replay evidence, so the episode cannot revert to `UNUSED`.
