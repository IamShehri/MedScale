...
`report-validation-contract.json` is also normative. It enforces exact bindings, canonical corpus item-ID membership, unique candidate identities, terminal error arithmetic, exclusion correspondence, aggregate/gate recomputation, and winner linkage that JSON Schema alone cannot express.
