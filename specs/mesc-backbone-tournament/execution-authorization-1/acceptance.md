...
Before activation, mechanically verify that `EXECUTION_CODE_SHA` resolves exactly to `EXECUTION_CODE_TREE`. Then, for every allowlist entry, resolve `path` against that exact commit/tree and require the resolved Git object to exist, be a blob, have Git tree mode exactly `100644` or `100755`, and have a Git blob SHA exactly equal to the entry's `git_blob_sha`. Tree mode `120000`, `160000`, `040000`, any other non-regular mode, or any non-blob object => `BLOCKED`.

The activation verifier must then acquire each allowlisted runtime executor/harness path descriptor-relatively from the exact read-only checkout root bound by `repository_checkout_sha` / `repository_checkout_tree`, using `openat2(2)` with `RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS | RESOLVE_NO_MAGICLINKS | RESOLVE_NO_XDEV` and `O_RDONLY | O_CLOEXEC | O_NOFOLLOW`, or a separately reviewed equivalent with the same atomic no-symlink/no-cross-mount guarantee. `fstat` on that exact opened descriptor must report a regular file. From the exact opened bytes, recompute the Git blob object ID using Git's canonical `blob <byte_length>\0<bytes>` framing and require equality with the allowlist `git_blob_sha`. Runtime execution/import must use that same immutable opened object or an object whose inode/bytes identity is mechanically proven equivalent on an immutable read-only mount immediately before execution/import. Any symlink substitution, checkout mutation, opened-byte/blob mismatch, object replacement, mutable alias, or inability to prove that executed/imported bytes are the verified bytes => `BLOCKED`.

The independently reviewed executable/imported executor-and-harness path set must equal the allowlist exactly: no reviewed executable path may be absent, no extra allowlist entry may exist, and no executor/harness file outside the allowlist may execute or be imported. Missing paths, duplicate paths, wrong object types or modes, Git blob mismatches, commit/tree mismatches, malformed canonical bytes, allowlist-digest mismatches, extra or omitted executable paths, or inability to reproduce any predicate => `BLOCKED`. The future executor/activation conformance suite must include negative fixtures for an allowlisted symlink/non-regular entry, mutation or replacement between verification and execution/import, and opened bytes whose recomputed Git blob ID differs from the allowlist; every such fixture must terminate as `BLOCKED`. The allowlist is therefore bound to `EXECUTION_CODE_SHA`/`EXECUTION_CODE_TREE` and to the exact runtime bytes; it must never be accepted as independent metadata.
...
repository_checkout_root_path = JSON string matching ^/[a-z0-9][a-z0-9._-]{0,127}(?:/[a-z0-9][a-z0-9._-]{0,127})*$
...
`repository_checkout_root_path` is a canonical absolute lowercase-ASCII path. After the leading `/`, every slash-separated component must match `^[a-z0-9][a-z0-9._-]{0,127}$`; therefore uppercase bytes, empty components, `.`, `..`, repeated separators, backslash, NUL, drive prefixes, alternate separators, Unicode, and components beginning with `.` are invalid. The verifier must reject invalid bytes rather than normalize, case-fold, collapse, resolve, or otherwise transform them. Bootstrap resolution must consume exactly the validated components in order; the bound string and resolved descriptor chain must represent the same path byte-for-byte.
...
The final `<ACTIVATION_ID>` directory under each trusted parent must not pre-exist. Directory creation plus descriptor acquisition must be protected against replacement as one security-critical lifecycle. A platform primitive that atomically creates the directory and returns a descriptor may be used when separately reviewed. Otherwise, the normative Linux fallback is permitted only when the activation process has mechanically proven exclusive mutation authority over the exact trusted parent for the entire `mkdirat`→`openat2` critical section: the parent must be inside an activation-private mount/user namespace or equivalent isolation boundary, no other process or principal may hold write/search capability sufficient to rename, unlink, replace, mount over, or recreate entries in that parent, and the activation sandbox must deny those mutation syscalls to every non-verifier process with access to the namespace during the critical section. Loss or inability to prove exclusive parent mutation authority => `BLOCKED`.

Under that protected critical section, create each `<ACTIVATION_ID>` directory exactly once descriptor-relatively; `EEXIST` or any pre-existing entry => `BLOCKED`. Immediately acquire the new root descriptor with `openat2(2)` using `RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS | RESOLVE_NO_MAGICLINKS | RESOLVE_NO_XDEV` and `O_PATH | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW`, require `fstat` to report a directory on the trusted parent's mount, and retain the parent descriptor and new root descriptor without releasing the mutation-exclusion guarantee until acquisition and identity checks complete. A separate adversary-visible create-then-path-reopen sequence without proven parent mutation exclusion is prohibited. Failure to establish either root descriptor exactly as specified => `BLOCKED` before model access.

The activation qualification suite must include adversarial concurrent replace-and-recreate fixtures for both activation parents. Under an intentionally unprotected writable parent, a replace/recreate attempt between creation and descriptor acquisition must demonstrate that the qualification refuses to claim safety; under the selected production isolation mechanism, the same mutation capability must be mechanically absent or denied. Any redirected descriptor, changed inode, unexpected replacement capability, or inability to prove the exclusion invariant => `BLOCKED`.
...
5. canonical executor implementation SHA/tree/blob allowlist and exact `EXECUTOR_ALLOWLIST_SHA256`, including duplicate-member-rejecting parsing, canonical reserialization equality, negative duplicate-member fixture PASS by reaching `BLOCKED`, proof that `EXECUTION_CODE_SHA` resolves to `EXECUTION_CODE_TREE`, every allowlisted path resolves at that exact commit/tree to tree mode `100644` or `100755` and the exact recorded Git blob SHA, the exact runtime-opened bytes recompute to that Git blob identity without symlink/non-regular substitution or mutation, the executed/imported bytes are the verified immutable bytes, the reviewed executable/imported path set equals the allowlist exactly, and the required symlink/mutation/blob-mismatch negative fixtures PASS by reaching `BLOCKED`;
...
8. exact canonical `RUNTIME_BINDING` bytes/schema/parser/reserialization, exact `runtime_binding_sha256`, exact runtime/container/dependency identities, exact canonical traversal-free repository checkout path and checkout SHA/tree, and exact pre-existing descriptor/bootstrap identity bindings;
...
12. exact `identity_preimage` scalar-schema validation, recomputation of every bound Git/hash/comment/runtime value, exact `ACTIVATION_ID` recomputation, regex validation, secure two-root bootstrap, proof of atomic create-and-return-descriptor semantics or equivalent exclusive parent-mutation authority across each create→descriptor-acquisition critical section, adversarial concurrent replace/recreate fixture PASS-by-`BLOCKED`, exact ASCII path-grammar/collision/no-cross-mount validation, closed filesystem-mutation-surface validation, and race-safe descriptor-relative artifact-root confinement proof;
