...
ATTESTATIONS SATISFIED; NO BLOCKING FINDINGS
...
Rejected P01-04B2B authorization attempt:
DRAFT PR #68 HEAD a309f0789c48646e36a87181b23673551a23d74d WAS REJECTED BY
INDEPENDENT EXACT-HEAD REVIEW — CHANGES REQUIRED; TWO BLOCKING FINDINGS;
NON-CANONICAL AND NEVER ADOPTED. A CORRECTED REPLACEMENT PACKAGE IS RECORDED
AT P01-T03B12.
...
Rejected predecessor attempt:
DRAFT PR #68 HEAD a309f0789c48646e36a87181b23673551a23d74d —
INDEPENDENT EXACT-HEAD REVIEW RETURNED CHANGES REQUIRED;
16 / 16 INDEPENDENCE ATTESTATIONS; 2 BLOCKING FINDINGS; 3 NON-BLOCKING
FINDINGS; NO MUTATION. NON-CANONICAL AND NEVER ADOPTED; NOT APPROVED.
...
BLOCKING-2:
CORRECTED — FINDING-ID PAYLOAD IS THE ACCEPTED B2A CANONICAL SINGLE-OBJECT
JSON SERIALIZATION OF AN EXACT SIX-MEMBER IDENTITY DOCUMENT
...
score_representation IS A FROZEN STRING — none, not_evaluable, OR
jaccard:<reduced_intersection>/<reduced_union>; REDUCED BY GCD; UNSIGNED
BASE-10 ASCII; NO LEADING ZEROS EXCEPT THE SINGLE DIGIT 0; DENOMINATOR
STRICTLY POSITIVE; 0 <= NUMERATOR <= DENOMINATOR; ZERO IS jaccard:0/1 AND A
FULL MATCH IS jaccard:1/1.
THE RUNTIME LeakageFinding.score FLOAT IS DERIVED, RUNTIME-ONLY AND
NON-AUTHORITATIVE; IT IS EXCLUDED FROM CANONICAL DOCUMENTS, CANONICAL BYTES,
FINGERPRINTS AND FINDING-ID PAYLOAD BYTES. A BINARY FLOAT IS NEVER PASSED TO
THE ACCEPTED B2A CANONICAL SERIALIZER.

Exact threshold comparison:
100 * INTERSECTION >= 90 * UNION FOR THE QUESTION THRESHOLD;
100 * INTERSECTION >= 95 * UNION FOR THE CONTEXT THRESHOLD.
NO ROUNDED OR BINARY-FLOATING-POINT VALUE MAY DETERMINE THRESHOLD PASSAGE.

Finding identity:
IDENTITY DOCUMENT HAS EXACTLY SIX FROZEN MEMBERS — schema, finding_type,
example_ids, source_document_ids, partitions, score_representation.
FINDING_IDENTITY_BYTES IS THE ACCEPTED B2A CANONICAL SINGLE-OBJECT JSON BYTE
SERIALIZATION OF THAT DOCUMENT, INCLUDING THE ACCEPTED TERMINAL-LINE-FEED
RULE AND FLOAT REJECTION.
finding_id = mesc-pilot-01-leakage-finding/1:sha256:<64-LOWERCASE-HEX SHA-256
OF FINDING_IDENTITY_BYTES>.
CALLER-SUPPLIED IDS ARE REGENERATED AND FAIL CLOSED ON MISMATCH.
...
Independent exact-head review of this replacement package:
OUTSTANDING — REQUIRED BEFORE READY OR MERGE.
THE PR #68 REVIEW DOES NOT CARRY OVER.
...
FD-B2B-10 for the P01-04B2B leakage primitive library, correcting the two
blocking defects identified in the rejected PR #68 head, together with the exact
