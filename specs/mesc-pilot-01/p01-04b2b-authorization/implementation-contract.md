...
| `token_set_jaccard` | two token sets | exact set Jaccard, §3–§5; **total**, including two empty sets | integer counts plus `score_representation` |
...
of token sets, including two empty sets, yields a defined result. No input pair
in the declared domain is left without a `score_representation`.

**Empty-input rules are authoritative exceptions and take precedence over the
general Jaccard fraction-construction rules of §5.**

### 4.1 Evaluation order — controlling
...
### 4.2 Decision table — controlling
...
```text
Both token sets empty (union zero):
intersection_size    = 0
union_size           = 0
score_representation = not_evaluable
runtime score        = null
near-duplicate threshold not passed
context-overlap threshold not passed
no jaccard fraction constructed
jaccard:0/0 prohibited
jaccard:0/1 not used for this case
...
Both normalized questions empty (caller-level, before Jaccard scoring):
no score is fabricated; the condition maps to empty_normalized_question;
initial classification is unresolved
```

The exactly-one-empty case is **policy-defined non-evaluable under senior
`FD-B2-6`**, which requires that no Jaccard score be fabricated when exactly one
token set is empty. The positive mathematical denominator does not override that
policy; the case is not mathematically undefined.

`jaccard:0/1` is limited to two non-empty disjoint token sets. It must never be
emitted when either token set is empty. Conversely, `not_evaluable` must never be
emitted for two non-empty disjoint token sets.

The token-set empty-input rules and the `empty_normalized_question` condition are
**distinct**. A punctuation-only or symbol-only question normalizes to a
non-empty string but tokenizes to an empty token set; when both sides do so, the
pair is scored by the union-zero rule and yields `not_evaluable`, **not**
...
not_evaluable                              — the governing empty-input policy
                                             prohibits creating a Jaccard score,
                                             or another authorized condition
                                             makes the comparison invalid
jaccard:<reduced_intersection>/<reduced_union>
                                           — both token sets non-empty
```

Rules:

- a `jaccard:<i>/<u>` representation is constructed **only when both token sets
  are non-empty** — §4 empty-input handling is evaluated first and controls;
...
| Case | `score_representation` |
|---|---|
| both non-empty, intersection 6 / union 9 | `jaccard:2/3` |
| both non-empty and disjoint | `jaccard:0/1` |
| both non-empty, intersection 9 / union 9 | `jaccard:1/1` |
| exact method | `none` |
...
already be in canonical lexicographic order **before** serialization. No
timestamp, path, hostname, username or other runtime metadata may enter the
identity document.

**Identity-array multiplicity.** `example_ids`, `source_document_ids` and
`partitions` are each a **unique-value list**. Duplicate values are invalid and
must never be silently removed or collapsed. After exact type and duplicate
validation, the unique values are sorted in canonical lexicographic order before
B2A canonical serialization. A duplicate value fails closed using the existing
typed invalid-finding-identifier error category. Input ordering alone is
non-semantic: permutations of the same unique values produce identical canonical
arrays, identical `FINDING_IDENTITY_BYTES`, and identical `finding_id` values.
Multiplicity is not semantic: a repeated value is invalid rather than
identity-bearing. This introduces no new dependency and no new public error
surface.
...
B2A canonicalization helpers. Manual concatenation and any alternate
...
```text
both token sets non-empty, 6 / 9   -> jaccard:2/3
both token sets non-empty, 9 / 9   -> jaccard:1/1
exact method                       -> none
non-evaluable                      -> not_evaluable

two non-empty disjoint token sets:
frozenset({"a"}), frozenset({"b"})
-> intersection 0
-> union 2
-> jaccard:0/1
...
The zero-score case must be instantiated with two **non-empty** disjoint token
sets. Instantiating it with an empty token set is prohibited — that input is
governed by §4 and yields `not_evaluable`.
...
Required identity-array multiplicity cases:

```text
permuted unique example_ids
-> identical sorted array and identical finding_id

permuted unique source_document_ids
-> identical sorted array and identical finding_id

permuted unique partitions
-> identical sorted array and identical finding_id

duplicate example_id          -> fail closed
duplicate source_document_id  -> fail closed
duplicate partition           -> fail closed
```

Also verify that **no silent deduplication** occurs: a duplicate value must
raise the typed invalid-finding-identifier error rather than being collapsed
into a valid unique array.
...
Required empty-input cases:

```text
Union zero — both token sets empty:
frozenset(), frozenset()
-> intersection_size 0
-> union_size 0
-> not_evaluable
-> runtime score null
-> no fraction constructed
-> neither threshold passes

punctuation-only or symbol-only normalized strings producing two empty
token sets
-> not_evaluable
-> not routed to empty_normalized_question
...
Exactly one token set empty, operand order reversed:
...
Union zero applies **only** when both token sets are empty. The
exactly-one-empty case is not a union-zero case; its union is positive.

Required non-empty zero-similarity case:
...
Required precedence case:

```text
An exactly-one-empty input must not use the general
intersection-zero / union-positive branch:
frozenset(), frozenset({"x"}) -> not_evaluable, never jaccard:0/1
```

Also verify:

```text
jaccard:0/0 is never constructed
jaccard:0/1 is not emitted when either token set is empty
not_evaluable is not emitted for two non-empty disjoint token sets
```
...
- lexicographic ID/source/partition normalization over unique values, with
  duplicates rejected rather than deduplicated;
